    ...
    def validate_dfs(self, data, raise_errors: bool = False) -> dict:
        ...
        if len(self.deltabelltitler) > 1:
            ...
            if not isinstance(data, pd.DataFrame):
                raise TypeError(
                    "Only one deltabell, expecting one pandas Dataframe in as your data."
                )
            # For the code below to access the data correctly
            to_validate = [data]
        else:
            validation_errors["deltabell_num"] = ValueError(
                "Deltabeller is shorter than one, weird. Make sure the uttaksbeskrivelse is ok."
            )
        ...
        for deltabell_num, deltabell in enumerate(self.variabler):
            ...
            if len(to_validate[deltabell_num].columns) != col_num:
                validation_errors[f"col_count_data_{deltabell_num}"] = ValueError(
                    f"Expecting {col_num} columns in datapart {deltabell_num}: {deltabell_navn}"
                )
        ...
        for kodeliste in self.kodelister:
            ...
            for kod in col_unique:
                if kod not in kod_unique:
                    categorycode_outside += [
                        f"Code {kod} in data, but not in uttrekksbeskrivelse, add to statbank admin? From column number {variabel['kolonnenummer']}, in deltabell number {deltabell_nr}, ({deltabell['deltabell']})"
                    ]
                ...
                if "Kodeliste_text" in variabel.keys():
                    if "format = " in variabel["Kodeliste_text"]:
                        ...
                        if not 1 == len(
                            ...
                            validation_errors[
                                f"time_single_length_format_{col_num}"
                            ] = ValueError(
                                f"Column number {col_num} does not have a single time format in the shape: {timeformat_raw}"
                            )
                        if (
                            ...
                            validation_errors[
                                f"time_formatlength_{col_num}"
                            ] = ValueError(
                                f"Column number {col_num} does not match time format in the shape: {timeformat_raw}"
                            )

                        timeformat = {
                            "nums": [
                                i for i, c in enumerate(timeformat_raw) if c.islower()
                            ],
                            "chars": {
                                i: c
                                for i, c in enumerate(timeformat_raw)
                                if c.isupper()
                            },
                            "specials": {
                                i: c
                        ...
                        if timeformat["nums"]:
                            for num in timeformat["nums"]:
                                if not all(
                                    ...
                                    validation_errors[
                                        f"time_non_digit_column{col_num}"
                                    ] = ValueError(
                        ...
                        if timeformat["chars"]:
                            for i, char in timeformat["chars"].items():
                                if not all(
                                    to_validate[i].iloc[:, col_num].str[i] == char
                                ):
                                    validation_errors[
                                        f"character_match_column{col_num}"
                                    ] = ValueError(
                        ...
                        if timeformat["specials"]:
                            for i, special in timeformat["specials"].items():
                                if not all(
                                    to_validate[i].iloc[:, col_num].str[i] == special
                                ):
                                    validation_errors[
