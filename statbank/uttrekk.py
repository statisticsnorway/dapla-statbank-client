    ...
    def transferdata_template(self, *dfs: list[pd.DataFrame] | pd.DataFrame) -> dict:
        ...
        if dfs:
            if not all([isinstance(df, pd.DataFrame) for df in dfs]):
                raise TypeError("All elements sent in to transferdata_template must be pandas dataframes.")
            if not len(dfs) == len(self.subtables):
                raise KeyError("Number of dataframes in must match the number of subtables.")
